 ...
 * </p>
 * <p>
 * This class reads an XML file conforming to the configure.dtd DTD and uses it
 * to configure and object by calling set, put or other methods on the object.
 * </p>
 * <p>
 * The actual XML file format may be changed (eg to spring XML) by implementing
 * the {@link ConfigurationProcessorFactory} interface to be found by the
 * {@link ServiceLoader} by using the DTD and first tag element in the file.
 * Note that DTD will be null if validation is off.
 * </p>
 * <p>
 * The configuration can be parameterised with properties that are looked up via
 * the Property XML element and set on the configuration via the map returned
 * from {@link #getProperties()}
 * </p>
 * <p>
 * The configuration can create and lookup beans by ID. If multiple
 * configurations are used, then it is good practise to copy the entries from
 * the {@link #getIdMap()} of a configuration to the next configuration so that
 * they can share an ID space for beans.
 * </p>
 */
public class XmlConfiguration {
	...
	private static final Class<?>[] __boxedPrimitives = { Boolean.class,
			Character.class, Byte.class, Short.class, Integer.class,
			Long.class, Float.class, Double.class, Void.class };

	private static final Class<?>[] __supportedCollections = { ArrayList.class,
			ArrayQueue.class, HashSet.class, Queue.class, List.class,
			Set.class, Collection.class, };

	private static final Iterable<ConfigurationProcessorFactory> __factoryLoader = ServiceLoader
			.load(ConfigurationProcessorFactory.class);
	...
	private static XmlParser initParser() {
		...
		URL config76 = Loader.getResource(XmlConfiguration.class,
				"org/eclipse/jetty/xml/configure_7_6.dtd");
		URL config90 = Loader.getResource(XmlConfiguration.class,
		...
		parser.redirectEntity("http://jetty.mortbay.org/configure.dtd",
				config90);
		parser.redirectEntity("http://jetty.eclipse.org/configure.dtd",
				config90);
		parser.redirectEntity("http://www.eclipse.org/jetty/configure.dtd",
				config90);

		parser.redirectEntity("-//Mort Bay Consulting//DTD Configure//EN",
				config90);
	 ...
	 * 
	 * @param configuration
	 *            the URL of the XML configuration
	 * @throws IOException
	 ...
	 *            preamble. The String should start with a
	 *            "&lt;Configure ....&gt;" element.
	 * @throws IOException
	 *             if the configuration could not be read
	 * @throws SAXException
	 ...
	 * 
	 * @param configuration
	 *            An input stream containing a complete configuration file
	 * @throws IOException
	 *             if the configuration could not be read
	 * @throws SAXException
	 *             if the configuration could not be parsed
	...
	private void setConfig(XmlParser.Node config) {
		if ("Configure".equals(config.getTag())) {
			...
			for (ConfigurationProcessorFactory factory : __factoryLoader) {
				_processor = factory.getConfigurationProcessor(_dtd,
						config.getTag());
				if (_processor != null)
					break;
			}
			...
			if (_processor == null)
				throw new IllegalStateException("Unknown configuration type: "
						+ config.getTag() + " in " + this);
		} else {
			throw new IllegalArgumentException("Unknown XML tag:"
					+ config.getTag());
		}
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the map of ID String to Objects that is used to hold and lookup any
	 * objects by ID.
	 * <p>
	 * A New, Get or Call XML element may have an id attribute which will cause
	 * the resulting object to be placed into this map. A Ref XML element will
	 * lookup an object from this map.
	 * </p>
	 * <p>
	 * When chaining configuration files, it is good practise to copy the ID
	 * entries from the ID map to the map of the next configuration, so that
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the map of properties used by the Property XML element to
	 ...
	 * 
	 * @param obj
	 *            The object to be configured, which must be of a type or super
	 *            type of the class attribute of the &lt;Configure&gt; element.
	 * @throws Exception
	...
	/**
	 * Applies the XML configuration script. If the root element of the
	 * configuration has an ID, an object is looked up by ID and its type
	 * checked against the root element's type. Otherwise a new object of the
	 * type specified by the root element is created.
	 * 
	 ...
	 * creates a new instance of an object before configuring it, so that a
	 * derived XmlConfiguration class may inject default values.
	 * 
	 * @param object
	...
	private static class JettyXmlConfiguration implements
			...
			if (obj == null && oClass != null) {
				...
				try {
					...
					throw new IllegalStateException(
							"No suitable constructor on " + oClass, x);
				}
			}
		...
		public void configure(Object obj, XmlParser.Node cfg, int i)
			...
			for (; i < cfg.size(); i++) {
				...
				try {
					...
					LOG.warn("Config error at " + node, e.toString() + " in "
							+ _url);
					throw e;
				}
			}
		...
		/*
		 * Call a set method. This method makes a best effort to find a matching
		 * set method. The type of the value is used to find a suitable set
		 * method by 1. Trying for a trivial type match. 2. Looking for a native
		 * type match. 3. Trying all correctly named methods for an auto
		 * conversion. 4. Attempting to construct a suitable value from original
		 * value. @param obj
		 * 
		 * @param node
		 */
		private void set(Object obj, XmlParser.Node node) throws Exception {
			...
			String name = "set"
					+ attr.substring(0, 1).toUpperCase(Locale.ENGLISH)
					+ attr.substring(1);
					...
					try {
						for (Class<?> c : __supportedCollections)
							if (paramTypes[0].isAssignableFrom(c)) {
								sets[s].invoke(obj,
										convertArrayToCollection(value, c));
								return;
							}
		 ...
		 * @return a collection of the desired type if the array can be
		 *         converted
		 */
		private static Collection<?> convertArrayToCollection(Object array,
			...
			if (collection == null)
				throw new IllegalArgumentException("Can't convert \""
						+ array.getClass() + "\" to " + collectionType);
			return collection;
		}
		...
		private void put(Object obj, XmlParser.Node node) throws Exception {
			if (!(obj instanceof Map))
				throw new IllegalArgumentException(
						"Object for put is not a Map: " + obj);
		...
		/*
		 * Call a method. A method is selected by trying all methods with
		 * matching names and number of arguments. Any object returned from the
		 * call is passed to the configure method to consume the remaining
		 * elements. Note that if this is a static call we consider only methods
		 * declared directly in the given class. i.e. we ignore any static
		 * methods in superclasses. @param obj
		 * 
		...
		private Object newArray(Object obj, XmlParser.Node node)
			...
			for (Object nodeObject : node) {
				...
				al = LazyList.add(al, (v == null && aClass.isPrimitive()) ? 0
						: v);
				if (nid != null)
		...
		/*
		 * Get the value of an element. If no value type is specified, then
		 * white space is trimmed out of the value. If it contains multiple
		 * value elements they are added as strings before being converted to
		 * any specified type. @param node
		 */
		...
		private static boolean isTypeMatchingClass(String type,
				Class<?> classToMatch) {
			return classToMatch.getSimpleName().equalsIgnoreCase(type)
					|| classToMatch.getName().equals(type);
		}
	...
	/**
	 * Run the XML configurations as a main application. The command line is
	 * used to obtain properties files (must be named '*.properties') and
	 ...
	 * <p>
	 * Each configuration file on the command line is used to create a new
	 * XmlConfiguration instance and the {@link XmlConfiguration#configure()}
	 * method is used to create the configured object. If the resulting object
	 * is an instance of {@link LifeCycle}, then it is started.
	 * <p>
	 * Any IDs created in a configuration are passed to the next configuration
	 * file on the command line using {@link #getIdMap()}. This allows objects
	 * with IDs created in one config file to be referenced in subsequent config
	 * files on the command line.
	 * 
	 * @param args
	 *            array of property and xml configuration filenames or
	 *            {@link Resource}s.
	 * @throws Exception
			...
			public Object run() {
				try {
					...
					try {
						...
						properties = (Properties) config.getMethod(
								"getProperties").invoke(null);
						LOG.debug(
								"org.eclipse.jetty.start.Config properties = {}",
								properties);
					...
					if (properties == null) {
						...
						Enumeration<?> ensysprop = System.getProperties()
								.propertyNames();
					...
					for (String arg : args) {
						if (arg.indexOf('=') >= 0) {
							int i = arg.indexOf('=');
							properties.put(arg.substring(0, i),
									arg.substring(i + 1));
						} else if (arg.toLowerCase(Locale.ENGLISH).endsWith(
								".properties"))
							properties.load(Resource.newResource(arg)
									.getInputStream());
					}
					...
					for (int i = 0; i < args.length; i++) {
						if (!args[i].toLowerCase(Locale.ENGLISH).endsWith(
								".properties")
								&& (args[i].indexOf('=') < 0)) {
							XmlConfiguration configuration = new XmlConfiguration(
									Resource.newResource(args[i]).getURL());
							if (last != null)
								configuration.getIdMap()
										.putAll(last.getIdMap());
