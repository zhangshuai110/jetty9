...
public class StartArgs {
	...
	/**
	 * JVM arguments, found via commmand line and in all active [exec] sections
	...
	/**
	 * List of all property references found directly on command line or
	 * start.ini
	 */
	...
	private void addUniqueXmlFile(String xmlRef, Path xmlfile)
			throws IOException {
	...
	private void addUniquePropertyFile(String propertyFileRef, Path propertyFile)
			throws IOException {
	...
	public void dumpActiveXmls(BaseHome baseHome) {
		...
		for (Path xml : xmls) {
			System.out.printf(" %s%n",
					baseHome.toShortForm(xml.toAbsolutePath()));
		}
	}
	...
	/**
	 * Ensure that the System Properties are set (if defined as a System
	 * property, or start.config property, or start.ini property)
	 * 
	 * @param key
	...
	/**
	 * Build up the Classpath and XML file references based on enabled Module
	 * list.
	 * 
	 * @param baseHome
	 * @param activeModules
	...
	public CommandLineBuilder getMainArgs(BaseHome baseHome, boolean addJavaInit)
			throws IOException {
		...
		if (dryRun || isExec()) {
			for (Prop p : properties)
				cmd.addRawArg(CommandLineBuilder.quote(p.key) + "="
						+ CommandLineBuilder.quote(p.value));
	...
	/**
	 * 得到main.class，默认是：org.eclipse.jetty.xml.XmlConfiguration
	 * 
	 * @return
	 */
	...
	/**
	 * 从上下文代码中可以看出，这里解析下面三个类： <br>
	 * CommandLineConfigSource<br>
	 * JettyBaseConfigSource<br>
	 ...
	 * @param replaceProps
	 *            true if properties in this parse replace previous ones, false
	 *            to not replace.
	 */
	private void parse(final String rawarg, String source, boolean replaceProps) {
		...
		StartLog.debug("parse(\"%s\", \"%s\", %b)", rawarg, source,
				replaceProps);
		...
		if ("--list-classpath".equals(arg) || "--version".equals(arg)
				|| "-v".equals(arg) || "--info".equals(arg)) {
			listClasspath = true;
			run = false;
			return;
		}
		...
		if (idx >= 0) {
			...
			if (replaceProps) {
				if (propertySource.containsKey(key)) {
					StartLog.warn("Property %s in %s already set in %s", key,
							source, propertySource.get(key));
				}
			...
			if ("OPTION".equals(key) || "OPTIONS".equals(key)) {
				...
				warn.append(key)
						.append(" no longer controls what libraries are on your classpath,");
		...
		throw new UsageException(ERR_BAD_ARG,
				"Unrecognized argument: \"%s\" in %s", arg, source);
	}
	...
	private void setProperty(String key, String value, String source,
		...
		if (key.equals("jetty.home")) {
			properties.setProperty("jetty.home",
					System.getProperty("jetty.home"), source);
			return;
		}

		// Special / Prevent override from start.ini's
		if (key.equals("jetty.base")) {
			properties.setProperty("jetty.base",
					System.getProperty("jetty.base"), source);
			return;
		}
