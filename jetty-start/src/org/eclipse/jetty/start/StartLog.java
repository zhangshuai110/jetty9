 ...
 * <p>
 * Because startup cannot rely on Jetty's Logging, an alternative logging is
 * established.
 * <p>
 * Optional behavior is to create a ${jetty.base}/logs/start.log with whatever
 * output the startup process produces.
 */
public class StartLog {
	...
	public void initialize(BaseHome baseHome,
			CommandLineConfigSource cmdLineSource) throws IOException {
		// 从命令行中获取debug信息
		...
		if (logfile != null) {
			try {
				...
				if (!FS.exists(startLog) && !FS.createNewFile(startLog)) {
					...
					throw new UsageException(UsageException.ERR_LOGGING,
							new IOException("Unable to create: "
									+ startLog.toAbsolutePath()));
				}

				if (!FS.canWrite(startLog)) {
					// Output about error is lost in majority of cases.
					throw new UsageException(UsageException.ERR_LOGGING,
							new IOException("Unable to write to: "
									+ startLog.toAbsolutePath()));
				}
				// TODO 这里的err是什么时候初始化的？
				err.println("StartLog to " + logfile);
				OutputStream fileout = Files.newOutputStream(startLog,
				...
				err.println("StartLog Establishing " + logfile + " on "
						+ new Date());
			} catch (IOException e) {
