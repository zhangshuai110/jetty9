 ...
 * <p>
 * By default, both <code>${jetty.home}</code> and <code>${jetty.base}</code>
 * are the same directory, but they can point at different directories.
 * <p>
 * The <code>${jetty.home}</code> directory is where the main Jetty binaries and
 * default configuration is housed.
 * <p>
 * The <code>${jetty.base}</code> directory is where the execution specific
 * configuration and webapps are obtained from.
 */
public class BaseHome {
	public static class SearchDir {
		...
		public String toShortForm(Path path) {
			...
			return String.format("${%s}%c%s", name, File.separatorChar,
					relative.toString());
		}
	}
	...
	private final static EnumSet<FileVisitOption> SEARCH_VISIT_OPTIONS = EnumSet
			.of(FileVisitOption.FOLLOW_LINKS);

	private final static int MAX_SEARCH_DEPTH = Integer.getInteger(
			"org.eclipse.jetty.start.searchDepth", 10);
	...
	/**
	 * 这个类首先创建累ConfigSources类 <br>
	 * 然后对日志文件进行初始化 <br>
	 * 然后再分别创建JettyHome和JettyBase的配置项类<br>
	 * 最后将JettyHome和JettyBase设置到SystemProperty中，
	 * 虽然这个工作和CommandLineConfigSource中的重复了
	 * 
	 * @param cmdLineSource
	 * @throws IOException
	 */
	public BaseHome(CommandLineConfigSource cmdLineSource) throws IOException {
		...
		// TODO this is cyclic construction as start log uses BaseHome, but
		// BaseHome constructor
		// calls other constructors that log. This appears to be a workable
		// sequence.
		StartLog.getInstance().initialize(this, cmdLineSource);
		// JettyBaseConfigSource和JettyHomeConfigSource比较类似，区别是前者解析start.ini文件，后者不解析
	...
	/**
	 * Create a {@link Path} reference to some content in
	 * <code>"${jetty.base}"</code>
	 * 
	 * @param path
	 ...
	 * <ol>
	 * <li>If provided path is an absolute reference., and exists, return that
	 * reference</li>
	 * <li>If exists relative to <code>${jetty.base}</code>, return that
	 * reference</li>
	 * <li>If exists relative to and <code>include-jetty-dir</code> locations,
	 * return that reference</li>
	 * <li>If exists relative to <code>${jetty.home}</code>, return that
	 * reference</li>
	 * <li>Return standard {@link Path} reference obtained from
	 * {@link java.nio.file.FileSystem#getPath(String, String...)} (no exists
	 * check performed)</li>
	 * </ol>
	 * 
	 * @param path
	 ...
	 */
	public List<Path> getPaths(Path dir, int searchDepth, String pattern)
			throws IOException {
	...
	/**
	 * Get a List of {@link Path}s from a provided pattern.<br>
	 * 根据提供的pattern得到一个路径列表。
	 * <p>
	 * Resolution Steps:
	 * <ol>
	...
	public List<Path> getPaths(String pattern) throws IOException {
		...
		if (PathMatchers.isAbsolute(pattern)) {
			...
			if (FS.isValidDirectory(root)) {
				...
				finder.setFileMatcher(matcher);
				finder.setBase(root);
				Files.walkFileTree(root, SEARCH_VISIT_OPTIONS,
						MAX_SEARCH_DEPTH, finder);
				hits.addAll(finder.getHits());
			}
		} else {
			...
			while (iter.hasPrevious()) {
				...
				if (source instanceof DirConfigSource) {
					...
					if (FS.isValidDirectory(deepDir)) {
						finder.setBase(dir);
						Files.walkFileTree(deepDir, SEARCH_VISIT_OPTIONS,
								MAX_SEARCH_DEPTH, finder);
					}
				}
			}
	 ...
	 * <code>"${jetty.home}"</code> or <code>"${jetty.base}"</code> where
	 * appropriate.
	 * 
	 * @param path
	 *            the path to shorten
			...
			if (source instanceof DirConfigSource) {
				...
				if (apath.startsWith(dir)) {
					if (dirsource.isPropertyBased()) {
						...
						return String.format("%s%c%s", dirsource.getId(),
								File.separatorChar, relative.toString());
					} else {
