 ...
 * <p>
 * This is larger in scope than the standard {@link java.util.Properties}, as it
 * will also handle tracking the origin of each property, if it was overridden,
...
public final class Props implements Iterable<Prop> {
	...
	public static String getValue(String arg) {
		...
		if (idx == (-1)) {
			throw new UsageException(ERR_BAD_ARG,
					"Argument is missing a required value: %s", arg);
		}
	...
	private Map<String, Prop> props = new TreeMap<>(
	...
	/**
	 * Add a potential argument as a property.<br>
	 * 如果arg以－D开头，则将同时设置到property和SystemProperty中。<br>
	 * 否则，只设置到property中。
	 * <p>
	 * If arg is not a property, ignore it.
	 * 
	 * @param arg
	 *            the argument to parse for a potential property
	 * @param source
	 *            the source for this argument (to track origin of property
	 *            from)
	 */
	...
	public static boolean hasPropertyKey(String name) {
		return Pattern.compile("(?<=[^$]|^)(\\$\\{[^}]*\\})").matcher(name)
				.find();
	}
