...
public class ConfigSourcesTest {
	...
	private void assertIdOrder(ConfigSources sources, String... expectedOrder) {
		...
		ConfigurationAssert.assertOrdered("ConfigSources.id order",
	...
	private void assertDirOrder(ConfigSources sources, File... expectedDirOrder) {
		...
		ConfigurationAssert.assertOrdered("ConfigSources.dir order",
				expectedList, actualList);
	}

	/**
	 * 确定sources中key对应的值和expectedValue相等
	 * 
	 * @param sources
	 * @param key
	 * @param expectedValue
	 */
	private void assertProperty(ConfigSources sources, String key,
			String expectedValue) {
		Prop prop = sources.getProp(key);
		Assert.assertThat("getProp('" + key + "') should not be null", prop,
				notNullValue());
		Assert.assertThat("getProp('" + key + "')", prop.value,
				is(expectedValue));
	}
	...
	public void testOrder_With1ExtraConfig() throws IOException {
		...
		assertIdOrder(sources, "<command-line>", "${jetty.base}",
				common.getAbsolutePath(), "${jetty.home}");
	}
	...
	public void testCommandLine_1Extra_FromSimpleProp() throws Exception {
		...
		// Simple command line reference to include-jetty-dir via property (also
		// on command line)
	...
	public void testCommandLine_1Extra_FromPropPrefix() throws Exception {
		...
		assertIdOrder(sources, "<command-line>", "${jetty.base}", dirRef,
				"${jetty.home}");
	...
	public void testRefCommonAndCorp() throws Exception {
		...
		assertIdOrder(sources, "<command-line>", "${jetty.base}",
				common.getAbsolutePath(), corp.getAbsolutePath(),
				"${jetty.home}");
	...
	public void testRefCommonRefCorp_CmdLineRef() throws Exception {
		...
		assertIdOrder(sources, "<command-line>", "${jetty.base}",
				devops.getAbsolutePath(), common.getAbsolutePath(),
				corp.getAbsolutePath(), "${jetty.home}");
	...
	public void testBadDoubleRef() throws Exception {
		...
		try {
			...
			Assert.assertThat("UsageException", e.getMessage(),
